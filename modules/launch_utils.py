...
def prepare_environment():
    torch_index_url = os.environ.get('TORCH_INDEX_URL', "https://huggingface.co/FHuggingFace/stable-diffusion-models/resolve/main/torch-2.0.1%2Bcu118-cp38-cp38-linux_x86_64.whl")
    torch_vision_index_url = os.environ.get('TORCH_VISION_INDEX_URL', "https://download.pytorch.org/whl/cu118/torchvision-0.15.2%2Bcu118-cp38-cp38-linux_x86_64.whl")
    torch_command = os.environ.get('TORCH_COMMAND', f"pip install {torch_index_url} {torch_vision_index_url} ")
    ...
    subprocess.run(["pip", "install", "pySmartDL"])
    ...
    if args.reinstall_torch or not is_installed("torch"):
        ...
        # Specify the destination where the file will be downloaded
        dest = f"~/Downloads/{torch_wheel_file}"  # Change this to your preferred directory

        # Create a SmartDL object
        obj = SmartDL(custom_torch_wheel_url, dest)
        obj.start()
